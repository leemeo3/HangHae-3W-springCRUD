...
public class BoardService {
    ...
    // 생성 로직
    public BoardResponseDto createBoard(BoardRequestDto requestDto) {       // requestDto를 받고 BoardResponseDto를 반환
        Board board = new Board(requestDto);                                // entity선언후 requestDto 삽입
        boardRepository.save(board);                                        // Repository에 entity 저장
        return new BoardResponseDto(board);                                 // entity -> Dto로 반환
    }
    // 조회 로직
    ...
    public ResponseDto deleteBoard(Long id, BoardRequestDto requestDto) {
        ...
        return new ResponseDto("삭제 성공");                         // entity -> Dto 반환
    }
}
